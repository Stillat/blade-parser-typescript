...
suite('Pint Transformer Accpetance: forms_resources_views_components_markdown_editor_blade_php', () => {
                ...
                <file-attachment directory>
                    <textarea
                        ...
                        @if (! $isConcealed())
                            {!! $isRequired() ? 'required' : null !!}
                        @endif
                        class="z-1 focus:border-primary-500 focus:ring-primary-500 absolute left-0 top-0 block h-full min-h-full w-full resize-none overflow-y-hidden whitespace-pre-wrap rounded-lg bg-transparent font-mono text-sm tracking-normal caret-black shadow-sm outline-none transition duration-75 focus:ring-1 focus:ring-inset"
                        x-bind:class="{
                            'dark:caret-white dark:focus:border-primary-500': @js(config('forms.dark_mode')),
                            'border-gray-300': ! (@js($getStatePath()) in $wire.__instance.serverMemo.errors),
                            'dark:border-gray-600': ! (@js($getStatePath()) in $wire.__instance.serverMemo.errors) && @js(config('forms.dark_mode')),
