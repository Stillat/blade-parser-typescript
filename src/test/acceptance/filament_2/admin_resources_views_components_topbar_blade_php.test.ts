...
suite('Pint Transformer Acceptance: admin_resources_views_components_topbar_blade_php', () => {
    setupTestHooks();
    test('pint: it can format admin_resources_views_components_topbar_blade_php', async () => {
        const input = `@props([
    'breadcrumbs' => [],
])
...
>
    <div class="flex w-full items-center px-2 sm:px-4 md:px-6 lg:px-8">
        <button
            ...
            x-on:click="$store.sidebar.isOpen ? $store.sidebar.close() : $store.sidebar.open()"
            @class([
                'filament-sidebar-open-button text-primary-500 focus:bg-primary-500/10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full outline-none hover:bg-gray-500/5',
