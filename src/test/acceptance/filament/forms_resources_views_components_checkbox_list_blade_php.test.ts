...
suite('Pint Transformer Acceptance: forms_resources_views_components_checkbox_list_blade_php', () => {
    setupTestHooks();
    test('pint: it can format forms_resources_views_components_checkbox_list_blade_php', async () => {
        ...
        @if (! $isDisabled)
            @if ($isSearchable)
                <input
                    ...
                    type="search"
                    placeholder="{{ $getSearchPrompt() }}"
                    class="focus:border-primary-500 focus:ring-primary-500 dark:focus:border-primary-500 mb-2 block h-7 w-full rounded-lg border-gray-300 px-2 text-sm text-gray-700 shadow-sm outline-none transition duration-75 focus:ring-1 focus:ring-inset dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                />
            @endif
