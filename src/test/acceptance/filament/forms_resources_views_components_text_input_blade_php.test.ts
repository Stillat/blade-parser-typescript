...
suite('Pint Transformer Acceptance: forms_resources_views_components_text_input_blade_php', () => {
    setupTestHooks();
    test('pint: it can format forms_resources_views_components_text_input_blade_php', async () => {
        const input = `@php
    ...
    >
        <input
            @if ($hasMask)
                ...
                wire:ignore
                @if ($isDebounced())
                    x-on:input.debounce.{{ $getDebounce() }}="$wire.$refresh"
                @endif
