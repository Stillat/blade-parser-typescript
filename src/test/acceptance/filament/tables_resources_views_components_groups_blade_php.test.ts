...
suite('Pint Transformer Acceptance: tables_resources_views_components_groups_blade_php', () => {
    test('pint: it can format tables_resources_views_components_groups_blade_php', () => {
        const input = `@props([
    'groups',
])
...
<div
    ...
    "
>
    <x-filament::dropdown {{ $attributes->class(['sm:hidden']) }} placement="bottom-start" shift wire:key="{{ $this->id }}.table.grouping">
        <x-slot name="trigger">
            <x-filament::icon-button
                icon="heroicon-m-rectangle-stack"
                icon-alias="tables::grouping.trigger"
