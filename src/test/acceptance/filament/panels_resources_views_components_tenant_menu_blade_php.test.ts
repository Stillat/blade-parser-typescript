...
suite('Pint Transformer Accpetance: panels_resources_views_components_tenant_menu_blade_php', () => {
    test('pint: it can format panels_resources_views_components_tenant_menu_blade_php', () => {
        const input = `@php
    ...
    <x-slot name="trigger">
        ...
        >
            ...
            >
                @if ($currentTenant instanceof \\Filament\\Models\\Contracts\\HasCurrentTenantLabel)
                    <p
                        class="text-[.625rem] font-bold uppercase tracking-wider text-gray-500 -mb-0.5 dark:text-gray-400"
                    >
                        {{ $currentTenant->getCurrentTenantLabel() }}
                    </p>
                @endif
